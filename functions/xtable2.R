...
mod.sum<-function(mod.list, 
     ...
     if(apsr==FALSE){
          ...
          stars <- ifelse(stars=="all",
                          paste("\\multicolumn{", 
                                1+length(mod.list),
                                "}{l}{\\footnotesize{\\emph{Notes:}  $^{**}p<0.01$; $^{*}p<0.05$; $^\\dagger p<0.1$ }} \\\\ \\multicolumn{",
                                1+length(mod.list),
                                "}{l}{\\footnotesize{", 
                                seNote,
                                "}}%",
...
xtable2<-function(mod.list, 
     ...
     if(coef){
          ...
          info <- mod.sum(info.list, apsr, stars, seNote, model.numbers, model.names, dep.varnames,k, length)
     }else{
          if(length(dep.varnames)==length(mod.list)){
               dep.varnames <- paste("\\multicolumn{1}{c}{", dep.varnames, "}")
          }
          ...
          info <- mod.sum(mod.list, apsr, stars, seNote, model.numbers, model.names, dep.varnames,k, length)
     }
